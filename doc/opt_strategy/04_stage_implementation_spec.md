...
>
> 文档状态：骨架已建立；后续按 Stage 分批补充并评审。未达到本文“可开工”条件的 Stage，不应直接进入编码。
...
| `04_stage_implementation_spec.md` | 具体改哪里、怎么实现、怎么验证 | 文件迁移、接口契约、实施步骤、测试命令、量化指标、证据和回滚 |
...
|--------|----------|-----------|---------------|--------------|
| P0-1 AmapService 结构化解析 | 1、3、4 | A | 待补齐 | `evidence/stage_1/`、`stage_3/`、`stage_4/` |
| P0-2 标准化数据调用链 | 5 | B | 待补齐 | `evidence/stage_5/` |
| P0-3 多偏好完整检索 | 5、6 | B | 待补齐 | `evidence/stage_5/`、`stage_6/` |
| P0-4 结构化草案输出 | 2、5、7B、8、9B | B | 待补齐 | 对应 Stage 目录 |
...
| P0-8 可执行时间轴信息 | 11、14 | B | 待补齐 | `stage_11/`、`stage_14/` |
...
确认状态：已确认。本文暂停在全局规范完成节点，尚未开始细化 Stage 1～5。
...
本次仅建立统一骨架和判定规则，不提前填写尚未完成代码基线核对的 Stage 细节。
